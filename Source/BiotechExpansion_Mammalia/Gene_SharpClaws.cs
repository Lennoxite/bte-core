...
namespace BTE
{
	public class Gene_SharpClaws : Gene
	{
		...
		public List<Hediff> LinkedHediff
		{
			get
			{
				...
				for (int i = 0; i < hediffs.Count; i++)
				{
					...
					if (hediffs[i].def == BTE_HediffDefOf.BTEMa_SharpClaws)
					{
		...
		public override void PostAdd()
		{
			...
			foreach (BodyPartRecord hd in hands)
			{
				...
				{
					Hediff h = this.pawn.health.AddHediff(BTE_HediffDefOf.BTEMa_SharpClaws, hd, null, null);
